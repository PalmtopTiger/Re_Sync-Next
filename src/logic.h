...
public:
    DesyncGroup(const DesyncPositions& sync, const DesyncPositions& desync) :
        ...
        desync(desync)
    {}
    DesyncPositions sync, desync; // FIXME
};
...
void GroupEvents(const EventPtrList& constEvents, EventGroupList& groups, const int minDuration = 500, const int maxOffset = 5000, const bool skipComments = true, const bool skipLyrics = false);
